...
INIT_PROMPT_SEED = """You are a hepatologist and endocrinologist with 10+ years of experience.
...
Output: JSON only
{
  ...
  "Predicted_subtype": "<Obesity-related / Hyperlipidemia-related / ALT-dominant / Mixed metabolic / Insulin resistance–related / Other>",
  "Fatty_Liver": <0 or 1>
}
"""
...
def validate_and_normalize_pred(data: Dict) -> Tuple[Dict, List[str]]:
    ...
    # Fatty_Liver -> int 0/1
    if "Fatty_Liver" in out:
        try:
            out["Fatty_Liver"] = int(out["Fatty_Liver"])
            out["Fatty_Liver"] = 1 if out["Fatty_Liver"] == 1 else 0
        except Exception:
            if "Fatty_Liver" not in missing:
                missing.append("Fatty_Liver")
...
def make_samples_from_grouped(grouped_dict_list, min_k=3, drop_keys=None):
    ...
    for patient in grouped_dict_list:
        ...
        for k in range(min_k, len(ps)+1):
            ...
            gold = ps[k-1].get("Fatty_Liver", None)
            if gold is None:
                continue
            try:
                gold = int(gold)
...
def gen_initial_prompts(client: OpenAI, model: str, batches: List[List[Tuple[List[Dict], int]]], num_candidates_per_batch=2):
    prompts = []
    sys = "You will propose concise system prompts that improve 0/1 prediction accuracy for fatty liver given k-1 check-ups."
    for bk in tqdm(batches):
        ...
        for (prefix, gold, meta) in bk[:4]:
            ...
            examples.append(ex)
        user = (
            "Given these examples, propose a system prompt that maximizes correctness of Fatty_Liver (0/1). "
        ...
        for ch in resp.choices:
            ...
            if ptxt:
                prompts.append(ptxt)
    prompts.append(INIT_PROMPT_SEED)
    return prompts
...
def refine_prompts_from_feedback(client: OpenAI, model: str, p_star: str, batch: List[Tuple[List[Dict], int]], num_candidates=3):
    ...
    fb_user = (
        ...
        "Focus on: TG↑, LDL/TC↑, HDL↓, Glucose↑, Weight↑ trends; handle noisy fluctuations; avoid over-relying on single visit.\n\n"
        f"Examples (JSON):\n"
